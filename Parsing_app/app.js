...
// pw: BkOEURxNL5ZabmDo
mongoose.connect("mongodb+srv://Parsing-app:BkOEURxNL5ZabmDo@cluster0.cxpu4.gcp.mongodb.net/funSystemParsingDB?retryWrites=true", { // mongodb cloude에 연결
...
const Program = function(title, department, date, remainDate, url, id, remainLabel) { // 펀시스템 프로그램 정보 저장할 객체의 생성자
  this.title = title;
...
async function checkFunSystem(pageNum) { // 숭실대 펀시스템 파싱하는 함수
  //console.log("checkFunSystem()");

  await getHTML(funSystemURL + pageNum).then(html => { // 펀시스템 pageNum번째 페이지의 HTML을 가져온다
    const $ = cheerio.load(html.data);

    $("ul.columns-4 li").each(function(index, element) {
      const remainLabel = $(element).find("label").attr("class"); // 종료 임박순 정렬을 위해 레이블 문자열 저장
      if (remainLabel === "CLOSED") { // 마감 생태인 프로그램을 읽었다면 그 뒤에 있는 프로그램들은 모두 마감상태이므로 더이상 읽을 필요 없음
        return false; // 파싱 종료
      }

      const title = $(element).find(".content .title").text(); // 프로그램 제목 읽어옴
      const department = $(element).find(".content .department").text().replace(/\s/g, ''); // 부서 읽어옴
      const tmpDate = $(element).find(".content small :nth-child(2)").text(); // 날짜 읽어옴
      const date = tmpDate.slice(15) + ' ~ ' + tmpDate.slice(15, tmpDate.length); // 읽어온 날짜 사이에 물결문자 넣어서 가독성 좋게 만든다
      const remainDate = $(element).find("label").first().find("b").text(); // 디데이 읽어옴
      const url = $(element).find("a").attr('href'); // 해당 프로그램 상세 페이지 주소 읽어옴
      // 상세페이지 주소에서 id 추출한다
      let tmpId = ""; // 임시로 id 저장할 변수
      let i = 21; // url의 21번째 문자부터가 id이기 때문에 21에서 시작
      while (true) {
        const convertedNum = Number(url[i]) // 해당 문자를 숫자형으로 변환한다
        if (convertedNum >= 0 && convertedNum <= 9) { // 숫자로 제대로 변환됐다면 id를 구성하던 문자가 맞으므로
          tmpId += url[i];
        } else { // 해당 문자가 숫자로 변환되지 않았다면 url에서 id는 모두 추출해 낸 것이므로 반복 종료
          break;
        }
        ++i;
      }
      ...
      const newProgram = new Program(title, department, date, remainDate, url, id, remainLabel) // 새로운 프로그램 정보 객체를 만들어 위에서 읽은 내용을 저장한다
      //console.log(newProgram);
      newProgramList.push(newProgram); // 새 프로그램 리스트에 추가한다

      if (index == 11) { // 펀시스템에는 한 페이지당 총 12개의 프로그램이 표시된다. 12번째(인덱스는 11) 프로그램을 확인했다면 해당 페이지는 모두 확인했으니 다음 페이지로 넘어간다.
...
function checkProgramsState(newProgramList) {
  ...
  getOldFunPrograms().then(function(oldProgramList) { //  DB에서 이전에 읽어왔던 프로그램 목록을 가져온 뒤,
    ...
    setTimeout(function() { // 위의 두 함수에서 프로그램 삭제/추가가 비동기적으로 이루어지므로 그 과정이 끝나기를 기다린 후 진행한다
      if (isListChanged) { // 프로그램 삭제 or 추가된 경우에
...
async function getOldFunPrograms() { // DB에 저장되어 있던 프로그램 리스트 가져오는 함수
...
function insertNewPrograms(oldProgramList, newProgramList) {
  ...
  if (!oldProgramList) { // DB에서 이전 프로그램 목록을 읽어오는데 실패했다면
  ...
  for (newProgram of newProgramList) {
    ...
    for (oldProgram of oldProgramList) {
      if (oldProgram.id == newProgram.id) { // 해당 프로그램이 이전에도 존재하던 프로그램이라면 (새로 등록된 프로그램이 아니라면)
        notFoundFlag = false;
        break; // 탐색 종료하고 다음 프로그램 검사하기 위해 break
      }
    }
    if (notFoundFlag) { // 새로 등록된 프로그램인 경우
      newProgram.isNewProgram = true;
      const funProgram = new FunProgram(newProgram);
      funProgram.save(function(err) { // DB에 추가
        if (err) {
          console.log(err);
...
function deleteClosedPrograms(oldProgramList, newProgramList) {
  ...
  for (oldProgram of oldProgramList) {
    let notFoundFlag = true;
    for (newProgram of newProgramList) {
      if (oldProgram.id == newProgram.id) { // 새로운 프로그램 목록에서도 동일한 프로그램이 발견됐다면
        notFoundFlag = false;
    ...
    if (notFoundFlag) { // 마감된(삭제된) 프로그램이라면
      FunProgram.deleteOne({ // DB에서 삭제
        id: oldProgram.id
      }, function(err) {
        if (err) {
          console.log(err);
...
function changeVersion() {
  ...
  Version.findOne({}, function(err, foundVersion) {
    if (!err) {
      ...
      console.log("UPDATE a version: " + newVersion);
    } else {
      console.log(err);
